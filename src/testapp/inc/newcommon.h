...
{
    swap_ether_addr(dpdk_pkt_offset(pkt, struct rte_ether_hdr *, OFFSET_ETHER));
}

static inline void dpdk_swap_ipv4_addr(struct rte_mbuf *pkt)
{
    swap_ipv4_addr(dpdk_pkt_offset(pkt, struct rte_ipv4_hdr *, OFFSET_IPV4));
}

static inline void dpdk_swap_udp_port(struct rte_mbuf *pkt)
{
    swap_udp_port(dpdk_pkt_offset(pkt, struct rte_udp_hdr *, OFFSET_UDP));
}

static inline uint16_t ipv4_hdr_checksum(struct rte_ipv4_hdr *ip_hdr)
{
...
{
    const struct rte_ether_hdr *ether_hdr = (struct rte_ether_hdr *)(frame + OFFSET_ETHER);
    const struct rte_ipv4_hdr *ipv4_hdr = (struct rte_ipv4_hdr *)(frame + OFFSET_IPV4);
    const struct rte_udp_hdr *udp_hdr = (struct rte_udp_hdr *)(frame + OFFSET_UDP);
