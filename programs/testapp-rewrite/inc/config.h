...
typedef char ipaddr_str[INET_ADDRSTRLEN]; /* String type that can contain an IPv4 address */
...
struct dpdk_conf
{
    ...
    enum comm_dir
    {
        ...
        DIRECTION_TX = 0x02,
        DIRECTION_TXRX = 0x03,
    } direction;               /* Indicates whether this application will only send, only receive, or both */
    struct rte_mempool *mbufs; /* Pointer to the mempool to take DPDK buffers from */
};
...
struct config
{
    ...
    bool silent;     /* Whether the application should print periodically data to standard output */
    bool touch_data; /* Whether the application should produce/consume each byte of the packet payload */
...
extern void config_initialize(struct config *conf, const struct config_defaults *defaults);
